...
describe InkscapeConverter do
  subject{InkscapeConverter.new('inkscape')}
  it 'should convert files using temp files' do
    ...
    expect(Tempfile).to receive(:new).with('temporary.svg').and_return(svg_file)
    expect(Tempfile).to receive(:new).with('temporary.pdf').and_return(pdf_file)

    expect(svg_file).to receive(:path).and_return('temporary.svg')
    expect(pdf_file).to receive(:path).and_return('temporary.pdf')
    expect(subject).to receive(:system).with('inkscape -T -f temporary.svg -A temporary.pdf')
    ...
    expect(subject.convert_to_pdf('svg')).to eq('my pdf')
  end
end
