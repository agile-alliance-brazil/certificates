...
describe CertificateOptionParser do
  ...
  it 'should provide filename prefix as given value' do
    options = subject.parse!(['data_folder', '--prefix', 'PREFIX-'])
    expect(options).to include(filename_prefix: 'PREFIX-')
  end
  it 'should provide filename prefix as nil if none provided' do
    options = subject.parse!(['data_folder'])
    expect(options).to include(filename_prefix: nil)
  end
